/*                                                                           */
/*   Item.cpp   アイテムの処理                                               */
/*                                                                           */
/*                                                                           */

#include "ssg/internal/Item.hpp"
#include "ssg/Gian.h"
#include "ssg/Hook.h"
#include "ssg/Sound.h"
#include "ssg/internal/Entity.hpp"
#include "ssg/internal/Fragment.hpp"
#include "ssg/internal/Maid.hpp"
#include "ssg/internal/PRankCtrl.hpp"
#include "ssg/internal/SEffect.hpp"
#include "hatoyama/logic/ut_math.h"

// アイテムを発生させる //
void C_ITEM::Set(int x, int y, uint8_t type)
{
	if(ItemNow+1>=ITEM_MAX) return;

	auto* ip = &Item[ItemInd[ItemNow++]];

	constexpr uint8_t deg = -64;

	ip->x     = x;
	ip->y     = y;
	ip->type  = type;	//ITEM_SCORE;
	ip->count = 0;

	switch(type){
		case(ITEM_SCORE):
			ip->vx    = cosl(deg,64*3);
			ip->vy    = sinl(deg,64*3);
		break;

		case(ITEM_EXTEND):
			ip->vx    = cosl(deg,64*2);
			ip->vy    = sinl(deg,64*2);
		break;

		case(ITEM_BOMB):
			ip->vx    = cosl(deg,64*2);
			ip->vy    = sinl(deg,64*2);
		break;
	}
}

// アイテムを動かす //
void C_ITEM::Move(void)
{
	int i, tx, ty, l;

	//point = 100+(Viv.evade)*100;
	const uint32_t point = ((((SY_MAX - Viv.y) >> 6) + (Viv.evade * 4)) * 160);

	for(i=0;i<ItemNow;i++){
		auto* ip = &Item[ItemInd[i]];
		if(!Viv.bomb_time){
			ip->x += ip->vx;
			ip->y += ip->vy;
		}
		else{
			tx = (Viv.x - ip->x);
			ty = (Viv.y - ip->y);
			l  = 1 + (isqrt(tx*tx + ty*ty) / 700);		// 512(3+6)
			ip->x += tx / l;
			ip->y += ty / l;
		}

		if(ip->vy<64*6) ip->vy += ITEM_GRAVITY;
		ip->count++;
		if(HITCHK(ip->x,Viv.x,ITEM_HITX) && HITCHK(ip->y,Viv.y,ITEM_HITY)){
			switch(ip->type){
				case(ITEM_SCORE):
					Hooks.Snd_SEPlay(SOUND_ID_SELECT, ip->x, false, &Hooks);
					PlayRank.Add((SY_MAX - Viv.y) >> 10);
					Viv.score_add(point);
					SEffect.SetPoint(ip->x, ip->y, point);
					if(Viv.evade){
						Fragment.Set(ip->x, ip->y, FRG_STAR3);
						Fragment.Set(ip->x, ip->y, FRG_STAR3);
					}
				break;

				case(ITEM_EXTEND):
					Hooks.Snd_SEPlay(SOUND_ID_SELECT, ip->x, false, &Hooks);
					SEffect.SetString((180 + 64), 80, "E x t e n d  !");
					Viv.left++;
				break;

				case(ITEM_BOMB):
					Hooks.Snd_SEPlay(SOUND_ID_SELECT, ip->x, false, &Hooks);
					SEffect.SetString(
						(120 + 64), 80, "B o m b   E x t e n d  !"
					);
					Viv.bomb++;
				break;
			}
			ip->type = ITEM_DELETE;
		}

		// 上方向では消去しない //
		if((ip->x)<GX_MIN-8*64||(ip->x)>GX_MAX+8*64||(ip->y)>GY_MAX+8*64)
			ip->type = ITEM_DELETE;
	}

	Indsort(ItemInd, ItemNow, Item, [](const ITEM_DATA& i) {
		return (i.type == ITEM_DELETE);
	});
}

// アイテム配列の初期化 //
void C_ITEM::IndSet(void)
{
	for(ITEM_DATA_IND i = 0; i < ITEM_MAX; i++) {
		ItemInd[i] = i;
		//memset(Item+i,0,sizeof(ITEM_DATA));
	}

	ItemNow = 0;
}
