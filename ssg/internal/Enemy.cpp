/*************************************************************************************************/
/*   ENEMY.C   敵の管理とか発生制御等                                                            */
/*                                                                                               */
/*************************************************************************************************/

#include "ssg/internal/Enemy.hpp"
#include "ssg/Gian.h"
#include "ssg/Hook.h"
#include "ssg/Round.h"
#include "ssg/Sound.h"
#include "ssg/internal/Boss.hpp"
#include "ssg/internal/ECLLen.hpp"
#include "ssg/internal/Effect3D.hpp"
#include "ssg/internal/Entity.hpp"
#include "ssg/internal/HLaser.hpp"
#include "ssg/internal/Item.hpp"
#include "ssg/internal/LLaser.hpp"
#include "ssg/internal/PRankCtrl.hpp"
#include "ssg/internal/RNG.hpp"
#include "hatoyama/logic/cast.h"
#include "hatoyama/logic/endian.h"
#include "hatoyama/logic/ut_math.h"

/*
 * ECLコマンドのアドレス更新には ECL_CmdLen[ECLコマンド定数] を使用する
 *
 */

// 左右反転用マクロ //
#define ABS_DEGRL(d)	((e->flag&EF_RLCHG) ? (128-(d)) : (d ))
#define ABS_VXRL(vx)	((e->flag&EF_RLCHG) ? (-(vx))   : (vx))
#define REL_DEGRL(d)	((e->flag&EF_RLCHG) ? (-(d ))   : (d ))


template <size_t N> void Indsort(
	std::array<uint16_t, N>& indices,
	uint16_t& count,
	const std::array<ENEMY_DATA, N>& entities
) {
	Indsort(indices, count, entities, [](const ENEMY_DATA& e) {
		return (e.flag & EF_DELETE);
	});
}

// 割り込みジャンプを調べる
void CheckECLInterrupt(const C_BOSS& Boss, ENEMY_DATA *e);

// ECLCST_?? からその値に変換する
static uint32_t ID2Value(const ENEMY_DATA *e, uint8_t id);



bool LaserHITCHK(const ENEMY_DATA *e, int ox, int oy, uint8_t d)
{
	const int chkw = (min(e->g_height, e->g_width) + (3 * 64));

	const int tx = (e->x - ox);
	const int ty = (e->y - oy);

	const int l = (cosl(d, tx) + sinl(d, ty));
	const int w = abs(-sinl(d, tx) + cosl(d, ty));

	return ((l > 0) && (w < chkw));
}

ENEMY_DATA* C_ENEMY::FindFree(void)
{
	if((EnemyNow + 1) >= ENEMY_MAX) {
		return nullptr;
	}
	return &Enemy[EnemyInd[EnemyNow++]];
}

template <uint8_t Count> static void SetSheet(
	ANIME_DATA& a, PIXEL_POINT topleft, PIXEL_SIZE size, uint8_t mode
)
{
	static_assert(Count <= ANIMEPTN_MAX);

	a.size = size;
	a.n    = Count;
	a.mode = mode;

	for(auto i = decltype(Count){ 0 }; i < Count; i++) {
		a.ptn[i] = PIXEL_LTWH{ topleft.x, topleft.y, size.w, size.h };
		topleft.x += size.w;
	}
}

// Cel sheets of quadratic sprites.
template <uint8_t Count, PIXEL_COORD Size> static void SetSheet(
	ANIME_DATA& a, PIXEL_POINT topleft, uint8_t mode
)
{
	SetSheet<Count>(a, topleft, { Size, Size }, mode);
}

// Rotational cel sheets always consist of 16 quadratic sprites.
template <PIXEL_COORD Size> static void SetSheetDeg(
	ANIME_DATA& a, PIXEL_POINT topleft
)
{
	SetSheet<16>(a, topleft, { Size, Size }, ANM_DEG);
}

// １パターンのグラフィックをアニメとして定義する //
static void SetAnimeRect2(ANIME_DATA& anm, int x1, int y1, int x2, int y2)
{
	anm.size = { (x2 - x1), (y2 - y1) };
	anm.n      = 1;
	anm.mode   = ANM_NORM;

	anm.ptn[0] = { x1, y1, x2, y2 };
}

bool C_ENEMY::Set(BUFFER_OWNED&& ecl, uint8_t stage)
{
	ECL_Head = std::move(ecl);
	if(!ECL_Head.get()) {
		return false;
	}

	switch(stage) {
	case(STAGE_EXTRA): // エキストラステージのグラフィック矩形
		// Extra Boss I //
		// 00 : ■Ａ　0 ～ 3   :  翼無し通常　（10fpp)
		SetSheet<4, 80>(Anime[0], {   0,   0 }, ANM_NORM);

		// 01 : ■Ｂ　4 ～ 7   :  翼有り通常　（10fpp)
		SetSheet<4, 80>(Anime[1], { 320,   0 }, ANM_NORM);

		// 02 : ■Ｃ　8 ～ 13  :  翼装着　（翼無し->有り）　（6fpp)
		SetSheet<6, 80>(Anime[2], {   0,  80 }, ANM_STOP);

		// 03 : ■Ｄ　14 ～ 15 :  翼有り時攻撃（移動無し）　（6fpp)
		SetSheet<2, 80>(Anime[3], { 480,  80 }, ANM_NORM);

		// 04 : ■Ｅ　16 ～ 17 :  翼装着時移動（もしくは移動攻撃）左　（6fpp)
		SetSheet<2, 80>(Anime[4], {   0, 160 }, ANM_NORM);

		// 05 : ■Ｆ　18 ～ 19 :  翼装着時移動（もしくは移動攻撃）右　（6fpp)
		SetSheet<2, 80>(Anime[5], { 160, 160 }, ANM_NORM);

		// 06 : ■Ｇ　24 ～ 30 :  段階変化　（翼有り->無し）　（6fpp)
		SetSheet<6, 80>(Anime[6], {   0, 240 }, ANM_STOP);

		// 07 : ■20 : 通常時ダメージ用マスク　（翼有り、無し兼用）
		SetSheet<1, 80>(Anime[7], { 320, 160 }, ANM_NORM);

		// 08 : ■21 : 停止攻撃時ダメージ用マスク　
		SetSheet<1, 80>(Anime[8], { 400, 160 }, ANM_NORM);

		// 09 : ■22 : 移動時（左）ダメージ用マスク　
		SetSheet<1, 80>(Anime[9], { 480, 160 }, ANM_NORM);

		// 10 : ■23 : 移動時（右）ダメージ用マスク　
		SetSheet<1, 80>(Anime[10], { 560, 160 }, ANM_NORM);

		SetSheet<4, 32>(Anime[11], {        0, (320 + (32 * 0)) }, ANM_NORM);
		SetSheet<4, 32>(Anime[12], {        0, (320 + (32 * 1)) }, ANM_NORM);
		SetSheet<4, 32>(Anime[13], {        0, (320 + (32 * 2)) }, ANM_NORM);
		SetSheet<4, 32>(Anime[14], {        0, (320 + (32 * 3)) }, ANM_NORM);
		SetSheet<4, 32>(Anime[15], {        0, (320 + (32 * 4)) }, ANM_NORM);
		SetSheet<4, 32>(Anime[16], { (32 * 4),  320             }, ANM_NORM);
		SetSheet<1, 32>(Anime[17], { (32 * 4), (320 + (32 * 1)) }, ANM_NORM);

		// Extra Boss II //
		// 18 : ■Ａ : 停止アニメ　（10～12fpp)
		SetSheet<4, 80>(Anime[18], {   0,  0 }, ANM_NORM);

		// 19 : ■Ｂ : 通常段階攻撃１　（?fpp)
		SetSheet<4, 80>(Anime[19], { 320,  0 }, ANM_STOP);

		// 20 : ■Ｃ :  通常段階攻撃２　および、高速移動前溜めポーズ　（6fpp)
		SetSheet<2, 80>(Anime[20], {   0, 80 }, ANM_NORM);

		// 21 : ■Ｄ : 魂状態（ショットに当たらない無敵） (1 ～ 2 fpp）
		// (160,80), (200,80), (240,80), (280,80)
		SetSheet<4, 40>(Anime[21], { 160, 80 }, ANM_NORM);

		// 22 : ■Ｅ : ダメージマスク(A)
		SetSheet<1, 80>(Anime[22], { 320, 80 }, ANM_NORM);

		// 23 : ■Ｅ : ダメージマスク(B)
		SetSheet<1, 80>(Anime[23], { 400, 80 }, ANM_NORM);

		// 24 : ■Ｅ : ダメージマスク(G)
		SetSheet<1, 80>(Anime[24], { 480, 80 }, ANM_NORM);

		// 25 : ■Ｅ : ダメージマスク(C)
		SetSheet<1, 80>(Anime[25], { 560, 80 }, ANM_NORM);

		// 26 : ■Ｆ : 高速移動アニメ
		Anime[26].size = { 80, 80 };
		Anime[26].n      = 16;
		Anime[26].mode   = ANM_DEG;		// 16 パターンで助かりましたな...
		for(const auto i : std::views::iota(0, 16)) {
			Anime[26].ptn[i] = PIXEL_LTWH{ ((i * 80) % 640), 160, 80, 80 };
		}

		// 27 : ■Ｇ : 通常段階攻撃２溜めポーズ　および、ワープ前後、
		SetSheet<1, 80>(Anime[27], { 560, 320 }, ANM_NORM);

		// 28-32 : 陰陽玉ｘ５
		SetSheet<8, 32>(Anime[28], {   0,  384       }, ANM_NORM);
		SetSheet<8, 32>(Anime[29], {   0, (384 + 32) }, ANM_NORM);
		SetSheet<8, 32>(Anime[30], {   0, (384 + 64) }, ANM_NORM);
		SetSheet<8, 32>(Anime[31], { 256, (384 + 32) }, ANM_NORM);
		SetSheet<8, 32>(Anime[32], { 256, (384 + 64) }, ANM_NORM);

		SetSheetDeg<32>(Anime[33], { 0,   0 });
		SetSheetDeg<32>(Anime[34], { 0,  32 });
		SetSheetDeg<32>(Anime[35], { 0,  64 });
		SetSheetDeg<32>(Anime[36], { 0,  96 });
		SetSheetDeg<32>(Anime[37], { 0, 128 });

		// レーザー発射物 //
		Anime[38].size = { 40, 56 };
		Anime[38].n      = 1;
		Anime[38].mode   = ANM_NORM;
		Anime[38].ptn[0] = PIXEL_LTWH{ 512, 0, 40, 56 };

		// 中ボス //
		Anime[39].size = { 72, 56 };
		Anime[39].n      = 2;
		Anime[39].mode   = ANM_NORM;
		Anime[39].ptn[0] = {  0, 424,  72     , 480 };
		Anime[39].ptn[1] = { 72, 424, (72 * 2), 480 };

		// 中ボスヒット //
		Anime[40].size = { 72, 56 };
		Anime[40].n      = 1;
		Anime[40].mode   = ANM_NORM;
		Anime[40].ptn[0] = { (72 * 2), 424, (72 * 3), 480 };

		// レーザー発射物ヒット //
		Anime[41].size = { 40, 64 };
		Anime[41].n      = 1;
		Anime[41].mode   = ANM_NORM;
		Anime[41].ptn[0] = PIXEL_LTWH{ 512, 56, 40, 56 };

		// 謎の光弾 //
		Anime[42].size = { 24, 24 };
		Anime[42].n      = 4;
		Anime[42].mode   = ANM_NORM;
		Anime[42].ptn[0] = PIXEL_LTWH{ 552,  0, 24, 24 };
		Anime[42].ptn[1] = PIXEL_LTWH{ 552, 24, 24, 24 };
		Anime[42].ptn[2] = PIXEL_LTWH{ 552,  0, 24, 24 };
		Anime[42].ptn[3] = PIXEL_LTWH{ 552, 48, 24, 24 };
		break;

	case(1): // Ｓｔａｇｅ１のグラフィック矩形
		// 中ボス //
		Anime[0].size = { 72, 56 };
		Anime[0].n      = 2;
		Anime[0].mode   = ANM_NORM;
		Anime[0].ptn[0] = {  0, 0,  72     , 56 };
		Anime[0].ptn[1] = { 72, 0, (72 * 2), 56 };

		SetSheetDeg<32>(Anime[1], { 0, (56 +  0) });
		SetSheetDeg<32>(Anime[2], { 0, (56 + 32) });
		SetSheetDeg<32>(Anime[3], { 0, (56 + 64) });
		SetSheetDeg<32>(Anime[4], { 0, (56 + 96) });

		// ボス //
		Anime[5].size = { 72, 64 };
		Anime[5].n      = 1;
		Anime[5].mode   = ANM_NORM;
		Anime[5].ptn[0] = { 0, 184, 72, 248 };

		// 中ボスフラッシュ用 //
		Anime[6].size = { 72, 56 };
		Anime[6].n      = 2;
		Anime[6].mode   = ANM_NORM;
		Anime[6].ptn[0] = { (72 * 2), 0, (72 * 3), 56 };
		Anime[6].ptn[1] = { (72 * 3), 0, (72 * 4), 56 };

		// ボスフラッシュ //
		Anime[7].size = { 72, 64 };
		Anime[7].n      = 1;
		Anime[7].mode   = ANM_NORM;
		Anime[7].ptn[0] = { 72, 184, (72 * 2), 248 };
		break;

	case(2): // Ｓｔａｇｅ２のグラフィック矩形
		SetSheetDeg<32>(Anime[0], { 0,   0 });
		SetSheetDeg<32>(Anime[1], { 0,  32 });
		SetSheetDeg<32>(Anime[2], { 0,  64 });
		SetSheetDeg<32>(Anime[3], { 0,  96 });
		SetSheetDeg<32>(Anime[4], { 0, 128 });

		Anime[5].size = { 112, 48 };
		Anime[5].n      = 1;
		Anime[5].mode   = ANM_NORM;
		Anime[5].ptn[0] = {   0, 160, 112, 208 };

		Anime[6].size = { 64, 48 };
		Anime[6].n      = 1;
		Anime[6].mode   = ANM_NORM;
		Anime[6].ptn[0] = { 112, 160, 176, 208 };

		// 中ボス //
		Anime[7].size = { 64, 64 };
		Anime[7].n      = 1;
		Anime[7].mode   = ANM_NORM;
		Anime[7].ptn[0] = {   0, 208,  64, 272 };

		// ボス羽 //
		Anime[8].size = { 112, 48 };
		Anime[8].n      = 1;
		Anime[8].mode   = ANM_NORM;
		Anime[8].ptn[0] = { 176, 160, 288, 208 };

		// ボス丸 //
		Anime[9].size = { 64, 48 };
		Anime[9].n      = 1;
		Anime[9].mode   = ANM_NORM;
		Anime[9].ptn[0] = { 288, 160, 352, 208 };

		// ボスフラッシュ１ //
		Anime[10].size = { 112, 48 };
		Anime[10].n      = 1;
		Anime[10].mode   = ANM_NORM;
		Anime[10].ptn[0] = { 176, (160 + 48), 288, (208 + 48) };

		// ボスフラッシュ２ //
		Anime[11].size = { 64, 48 };
		Anime[11].n      = 1;
		Anime[11].mode   = ANM_NORM;
		Anime[11].ptn[0] = { 288, (160 + 48), 352, (208 + 48) };

		// 中ボスフラッシュ //
		Anime[12].size = { 64, 64 };
		Anime[12].n      = 1;
		Anime[12].mode   = ANM_NORM;
		Anime[12].ptn[0] = { (0 + 64), 208, (64 + 64), 272 };
/*
		// 羽モノ Left-I //
		Anime[10].size = { 104, 72 };
		Anime[10].n      = 1;
		Anime[10].mode   = ANM_NORM;
		Anime[10].ptn[0] = { 184, 208, 288, 280 };

		// 羽モノ Right-I //
		Anime[11].size = { 104, 72 };
		Anime[11].n      = 1;
		Anime[11].mode   = ANM_NORM;
		Anime[11].ptn[0] = { 288, 208, 392, 280 };

		// 羽モノ Left-0 //
		Anime[12].size = { 88, 80 };
		Anime[12].n      = 1;
		Anime[12].mode   = ANM_NORM;
		Anime[12].ptn[0] = { 200, 280, 288, 360 };

		// 羽モノ Right-0 //
		Anime[13].size = { 88, 80 };
		Anime[13].n      = 1;
		Anime[13].mode   = ANM_NORM;
		Anime[13].ptn[0] = { 288, 280, 376, 360 };
*/
		SetAnimeRect2(Anime[14],   0, 288, 159, 479);	// 雲
		SetAnimeRect2(Anime[15], 160, 384, 271, 479);	//
		SetAnimeRect2(Anime[16], 272, 368, 390, 478);	//
		SetAnimeRect2(Anime[17], 400, 368, 496, 431);	//
		SetAnimeRect2(Anime[18], 400, 160, 558, 359);	//
		SetAnimeRect2(Anime[19], 528,  48, 639, 160);	//
		SetAnimeRect2(Anime[20], 560, 160, 639, 270);	//
		SetAnimeRect2(Anime[21], 576, 320, 639, 399);	//
		break;

	case(3): // ゲイツ殿のステージ
		Anime[0].size = { 56, 56 };
		Anime[0].n      = 16;
		Anime[0].mode   = ANM_DEG;
		for(const int i : std::views::iota(0, 8)) {
			Anime[0].ptn[i    ] = PIXEL_LTWH{ i*56,  0, 56, 56 };
			Anime[0].ptn[i + 8] = PIXEL_LTWH{ i*56, 56, 56, 56 };
		}

		SetSheetDeg<32>(Anime[1], { 0, 112 });
		SetSheetDeg<32>(Anime[2], { 0, 144 });
		SetSheetDeg<32>(Anime[3], { 0, 176 });

		Anime[4].size = { 48, 16 };
		Anime[4].n      = 2;
		Anime[4].mode   = ANM_NORM;
		Anime[4].ptn[0] = PIXEL_LTWH{ 592,  0, 48, 16 };
		Anime[4].ptn[1] = PIXEL_LTWH{ 592, 16, 48, 16 };

		Anime[5].size = { 48, 16 };
		Anime[5].n      = 2;
		Anime[5].mode   = ANM_NORM;
		Anime[5].ptn[0] = PIXEL_LTWH{ 592, 32, 48, 16 };
		Anime[5].ptn[1] = PIXEL_LTWH{ 592, 48, 48, 16 };

		// ボス (464,384)
		Anime[6].size = { 11*16, 5*16 + 8 };
		Anime[6].n      = 1;
		Anime[6].mode   = ANM_NORM;
		Anime[6].ptn[0] = PIXEL_LTWH{ 464, 392, (11 * 16), ((5 * 16) + 8) };

		SetSheetDeg<32>(Anime[7], { 0, 208 });
		SetSheetDeg<40>(Anime[8], { 0, 240 });

		// ボスの影 //
		Anime[10].size = { 196, 100 };
		Anime[10].n      = 1;
		Anime[10].mode   = ANM_NORM;
		Anime[10].ptn[0] = { 444, 292, 640, 392 };

		// ボスフラッシュ
		Anime[9].size = { 128, 76 };
		Anime[9].n      = 1;
		Anime[9].mode   = ANM_NORM;
		Anime[9].ptn[0] = {  512, 164, 640, 240 };
	/*	Anime[9].size = { (11 * 16), ((5 *16) + 8) };
		Anime[9].n      = 1;
		Anime[9].mode   = ANM_NORM;
		Anime[9].ptn[0] = PIXEL_LTWH{ 464, (392 - 88), (11 * 16), ((5 * 16) + 8) };
	*/
		break;

	case(4): // マリーさんのステージ
		SetSheetDeg<32>(Anime[0], {  0,   0 });
		SetSheetDeg<32>(Anime[1], {  0,  32 });
		SetSheetDeg<32>(Anime[2], {  0,  64 });
		SetSheet<2, 32>(Anime[3], {  0,  96 }, ANM_NORM);
		SetSheetDeg<24>(Anime[4], { 64,  96 });
		SetSheetDeg<32>(Anime[5], {  0, 128 });

		//(304,296)-(640,480)
		Anime[6].size = { (640 - 304), (480 - 296) };
		Anime[6].n      = 1;
		Anime[6].mode   = ANM_NORM;
		Anime[6].ptn[0] = { 304, 296, 640, 480 };

		// ボスのフラッシュ //
		Anime[7].size = { (640 - 304 - 32), (480 - 296) };	// ここに注意
		Anime[7].n      = 1;
		Anime[7].mode   = ANM_NORM;
		Anime[7].ptn[0] = {   0, 296, 304, 480 };
		break;

	case(5): // ご主人様のステージ
		SetSheetDeg<32>(Anime[0], {   0,   0 });	// 赤いヤツ
		SetSheetDeg<32>(Anime[1], {   0,  32 });	// 赤いヤツの出現用演出
		SetSheetDeg<32>(Anime[2], {   0,  64 });	// 蒼いヤツ
		SetSheetDeg<32>(Anime[3], {   0,  96 });	// 緑のヤツ
		SetSheetDeg<32>(Anime[4], {   0, 128 });	// オレンジなやつ
		SetSheet<4, 32>(Anime[5], { 512,   0 }, ANM_NORM);	// 原子炉搭載ビット
		SetSheet<4, 32>(Anime[6], { 512,  64 }, ANM_NORM);	// オレンジなヤツの出現用演出

		// 中ボス用のオプション //
		Anime[7].size = { 24, 24 };
		Anime[7].n      = 4;
		Anime[7].mode   = ANM_NORM;
		Anime[7].ptn[0] = PIXEL_LTWH{ 592, (96 +  0), 24, 24 };
		Anime[7].ptn[1] = PIXEL_LTWH{ 592, (96 + 24), 24, 24 };
		Anime[7].ptn[2] = PIXEL_LTWH{ 592, (96 +  0), 24, 24 };
		Anime[7].ptn[3] = PIXEL_LTWH{ 592, (96 + 48), 24, 24 };

		SetSheet<1>(Anime[8], { 512, 96 }, { 80, 72 }, ANM_NORM); // 屈強なる中ボス

		// メタリックご主人 //
		SetSheet<1>(Anime[9], { 304, 256 }, { 336, 224 }, ANM_NORM);
		break;

	case(6):
		// ラスボス(座り->立ち) //
		Anime[0].size = { 56, 72 };
		Anime[0].n      = 6;
		Anime[0].mode   = ANM_STOP;
		Anime[0].ptn[0] = PIXEL_LTWH{ (56 * 0), 72, 56, 72 };
		Anime[0].ptn[1] = PIXEL_LTWH{ (56 * 1), 72, 56, 72 };
		Anime[0].ptn[2] = PIXEL_LTWH{ (56 * 2), 72, 56, 72 };
		Anime[0].ptn[3] = PIXEL_LTWH{ (56 * 3), 72, 56, 72 };
		Anime[0].ptn[4] = PIXEL_LTWH{ (56 * 4), 72, 56, 72 };
		Anime[0].ptn[5] = PIXEL_LTWH{ (56 * 5), 72, 56, 72 };

		// ラスボス(立ち->座り) //
		Anime[1].size = { 56, 72 };
		Anime[1].n      = 6;
		Anime[1].mode   = ANM_STOP;
		Anime[1].ptn[0] = PIXEL_LTWH{ (56 * 5), 72, 56, 72 };
		Anime[1].ptn[1] = PIXEL_LTWH{ (56 * 4), 72, 56, 72 };
		Anime[1].ptn[2] = PIXEL_LTWH{ (56 * 3), 72, 56, 72 };
		Anime[1].ptn[3] = PIXEL_LTWH{ (56 * 2), 72, 56, 72 };
		Anime[1].ptn[4] = PIXEL_LTWH{ (56 * 1), 72, 56, 72 };
		Anime[1].ptn[5] = PIXEL_LTWH{ (56 * 0), 72, 56, 72 };

		// ラスボス(ガード) //
		Anime[2].size = { 56, 72 };
		Anime[2].n      = 4;
		Anime[2].mode   = ANM_NORM;
		Anime[2].ptn[0] = PIXEL_LTWH{ (56 * 6), 72, 56, 72 };
		Anime[2].ptn[1] = PIXEL_LTWH{ (56 * 7), 72, 56, 72 };
		Anime[2].ptn[2] = PIXEL_LTWH{ (56 * 6), 72, 56, 72 };
		Anime[2].ptn[3] = PIXEL_LTWH{ (56 * 8), 72, 56, 72 };

		// ラスボス(攻撃-壱) //
		Anime[3].size = { 56, 72 };
		Anime[3].n      = 9+1;
		Anime[3].mode   = ANM_STOP;
		Anime[3].ptn[0] = PIXEL_LTWH{ (56 * 0),  0, 56, 72 };
		Anime[3].ptn[1] = PIXEL_LTWH{ (56 * 1),  0, 56, 72 };
		Anime[3].ptn[2] = PIXEL_LTWH{ (56 * 2),  0, 56, 72 };
		Anime[3].ptn[3] = PIXEL_LTWH{ (56 * 3),  0, 56, 72 };
		Anime[3].ptn[4] = PIXEL_LTWH{ (56 * 4),  0, 56, 72 };
		Anime[3].ptn[5] = PIXEL_LTWH{ (56 * 5),  0, 56, 72 };
		Anime[3].ptn[6] = PIXEL_LTWH{ (56 * 6),  0, 56, 72 };
		Anime[3].ptn[7] = PIXEL_LTWH{ (56 * 7),  0, 56, 72 };
		Anime[3].ptn[8] = PIXEL_LTWH{ (56 * 8),  0, 56, 72 };
		Anime[3].ptn[9] = PIXEL_LTWH{ (56 * 5), 72, 56, 72 }; 	// ちょっと追加

		// 幼虫期(//
		SetAnimeRect2(Anime[4], 432, 272, 632, 464);

		// ラスボス(ジャンプに見えるといいなぁ) //
		Anime[5].size = { 56, 72 };
		Anime[5].n      = 11;
		Anime[5].mode   = ANM_STOP;
		Anime[5].ptn[ 0] = PIXEL_LTWH{ (56 * 0), 72, 56, 72 };
		Anime[5].ptn[ 1] = PIXEL_LTWH{ (56 * 1), 72, 56, 72 };
		Anime[5].ptn[ 2] = PIXEL_LTWH{ (56 * 2), 72, 56, 72 };
		Anime[5].ptn[ 3] = PIXEL_LTWH{ (56 * 3), 72, 56, 72 };
		Anime[5].ptn[ 4] = PIXEL_LTWH{ (56 * 4), 72, 56, 72 };
		Anime[5].ptn[ 5] = PIXEL_LTWH{ (56 * 5), 72, 56, 72 };
		Anime[5].ptn[ 6] = PIXEL_LTWH{ (56 * 4), 72, 56, 72 };
		Anime[5].ptn[ 7] = PIXEL_LTWH{ (56 * 3), 72, 56, 72 };
		Anime[5].ptn[ 8] = PIXEL_LTWH{ (56 * 2), 72, 56, 72 };
		Anime[5].ptn[ 9] = PIXEL_LTWH{ (56 * 1), 72, 56, 72 };
		Anime[5].ptn[10] = PIXEL_LTWH{ (56 * 0), 72, 56, 72 };


		// 蝶状態で放つビット？(Open) //
		Anime[6].size = { 33, 32 };
		Anime[6].n      = 10;
		Anime[6].mode   = ANM_STOP;
		Anime[6].ptn[0] = PIXEL_LTWH{ (32 * 0), 416, 32, 32 };
		Anime[6].ptn[1] = PIXEL_LTWH{ (32 * 1), 416, 32, 32 };
		Anime[6].ptn[2] = PIXEL_LTWH{ (32 * 2), 416, 32, 32 };
		Anime[6].ptn[3] = PIXEL_LTWH{ (32 * 3), 416, 32, 32 };
		Anime[6].ptn[4] = PIXEL_LTWH{ (32 * 4), 416, 32, 32 };
		Anime[6].ptn[5] = PIXEL_LTWH{ (32 * 5), 416, 32, 32 };
		Anime[6].ptn[6] = PIXEL_LTWH{ (32 * 0), 448, 32, 32 };
		Anime[6].ptn[7] = PIXEL_LTWH{ (32 * 1), 448, 32, 32 };
		Anime[6].ptn[8] = PIXEL_LTWH{ (32 * 2), 448, 32, 32 };
		Anime[6].ptn[9] = PIXEL_LTWH{ (32 * 3), 448, 32, 32 };

		// 蝶状態で放つビット？(Close) //
		Anime[7].size = { 33, 32 };
		Anime[7].n      = 10;
		Anime[7].mode   = ANM_STOP;
		Anime[7].ptn[0] = PIXEL_LTWH{ (32 * 3), 448, 32, 32 };
		Anime[7].ptn[1] = PIXEL_LTWH{ (32 * 2), 448, 32, 32 };
		Anime[7].ptn[2] = PIXEL_LTWH{ (32 * 1), 448, 32, 32 };
		Anime[7].ptn[3] = PIXEL_LTWH{ (32 * 0), 448, 32, 32 };
		Anime[7].ptn[4] = PIXEL_LTWH{ (32 * 5), 416, 32, 32 };
		Anime[7].ptn[5] = PIXEL_LTWH{ (32 * 4), 416, 32, 32 };
		Anime[7].ptn[6] = PIXEL_LTWH{ (32 * 3), 416, 32, 32 };
		Anime[7].ptn[7] = PIXEL_LTWH{ (32 * 2), 416, 32, 32 };
		Anime[7].ptn[8] = PIXEL_LTWH{ (32 * 1), 416, 32, 32 };
		Anime[7].ptn[9] = PIXEL_LTWH{ (32 * 0), 416, 32, 32 };

		SetSheet<1>(Anime[8], { 0, 368 }, { 48, 48 }, ANM_NORM);	// 屈強なる中ボス
		break;
	}

	return true;
}

void C_ENEMY::Move(void)
{
	int				i;//,chkx,chky;

	if(Boss.NumAlive() == 0) {
		Tama.ResetHoming();
	}

	for(i=0;i<EnemyNow;i++){
		auto* e = &Enemy[EnemyInd[i]];
		e->IsDamaged = 0;
		if(!(e->flag & EF_BOMB)){
			// 通常の敵の処理 //
			ECL_Parse(e);

			// 弾発射モードによる分岐 //
			if(e->t_rep && e->hp){
				e->tama_c = (e->tama_c+1)%(e->t_rep);
				if(e->tama_c==0){
					auto& TamaCmd = Tama.TamaCmd;
					TamaCmd = e->t_cmd;
					TamaCmd.x += e->x;
					TamaCmd.y += e->y;
					Tama.Set();
				}
			}

			// サボテンヒットチェック //
			if(HITCHK(e->x,Viv.x,e->g_width) && HITCHK(e->y,Viv.y,e->g_height) && Viv.muteki==0){
				// ここら辺で敵にダメージを与えるとおもしろいかも？ //
				if(e->flag & EF_HITSB) {
					Viv.Dead();
				}
			}

			// 範囲外チェック //
			if((e->y<GY_MIN-(e->g_height))||(e->y>GY_MAX+(e->g_height))||
				(e->x<GX_MIN-(e->g_width))||(e->x>GX_MAX+(e->g_width))){
				if((e->flag&EF_CLIP)==0){
					Delete(*e);
				}
			}
		}
		else if(e->count>=8*ENEMY_BOMB_SPD-1){
			e->flag = EF_DELETE;
		}

		// ホーミングの準備 //
		if((Boss.NumAlive() == 0) && (e->flag & EF_DAMAGE)) {
			Tama.UpdateHoming({ &e->x, &e->y });
		}

		// アニメーションの動作 //
		AnimeMove(e);

		e->count++;
	}

	Indsort(EnemyInd, EnemyNow, Enemy);
}

// 雑魚を消滅させる //
void C_ENEMY::Clear(void)
{
	int			i;

	for(i=0;i<EnemyNow;i++){
		auto* e = &Enemy[EnemyInd[i]];
		if(e->flag == EF_BOMB) continue;

		if(e->flag & EF_DRAW){
			this->Explode(*e);
			Hooks.Snd_SEPlay(SOUND_ID_BOMB, e->x, false, &Hooks);
		}
		else{
			// 描画しないタイプの敵の消去の仕方は他の場合と異なり、 //
			// 爆発アニメ・爆発音を再生しない                       //
			this->Delete(*e);
			e->hp    = 0;
			e->count = 0;
			// 爆発音の再生は行わない //
		}
	}

	Indsort(EnemyInd, EnemyNow, Enemy);
}

void C_ENEMY::IndSet(void)
{
	int i;

	for(i=0;i<ENEMY_MAX;i++){
		//memset(Enemy+i,0,sizeof(ENEMY_DATA));
		EnemyInd[i] = i;
	}

	EnemyNow = 0;
}

bool C_ENEMY::DamageApply(ENEMY_DATA& e, int damage)
{
	e.IsDamaged = ((e.count) & 1);
	if(e.hp <= damage) {
		Hooks.Snd_SEPlay(SOUND_ID_BOMB, e.x, false, &Hooks);
		Viv.PowerUp(static_cast<uint8_t>(e.hp)); // パワーアップ
		this->Explode(e);
		Viv.score_add(e.score);
		if(e.item) {
			Item.Set(e.x, e.y, e.item);
		}
	} else {
		Hooks.Snd_SEPlay(SOUND_ID_HIT, e.x, false, &Hooks);
		Viv.PowerUp(damage); // ここでもパワーアップ
		e.hp -= damage;
	}
	return true;
}

bool C_ENEMY::Damage(int x, int y, int damage)
{
	int				i;

	if(Boss.Damage(x, y, damage)) {
		return true;
	}

	for(i=0;i<EnemyNow;i++){
		auto* e = &Enemy[EnemyInd[i]];
		if(HITCHK(x,e->x,e->g_width) && HITCHK(y,e->y,e->g_height) && (e->flag&EF_DAMAGE)){
			if(e->flag==EF_BOMB || !(e->flag&EF_DAMAGE)) continue;
			else{
				return this->DamageApply(*e, damage);
			}
		}
	}

	return false;
}

bool C_ENEMY::Damage2(int x, int y, int damage)
{
	int				i;
	auto ret_val = Boss.Damage2(x, y, damage);

	for(i=0;i<EnemyNow;i++){
		auto* e = &Enemy[EnemyInd[i]];
		if(HITCHK(x,e->x,e->g_width) && (y > e->y) && (e->flag&EF_DAMAGE)){
			if(e->flag==EF_BOMB || !(e->flag&EF_DAMAGE)) continue;
			else{
				ret_val = this->DamageApply(*e, damage);
			}
		}
	}

	return ret_val;
}

// ナナメレーザーの当たり判定 //
void C_ENEMY::Damage3(int x, int y, uint8_t d)
{
	int				i;
	// bool	ret_val = false;
	constexpr int damage = 8;

	Boss.Damage3(x, y, d);

	for(i=0;i<EnemyNow;i++){
		auto* e = &Enemy[EnemyInd[i]];
		if(LaserHITCHK(e, x, y, d) && (e->flag&EF_DAMAGE)){
			if(e->flag==EF_BOMB || !(e->flag&EF_DAMAGE)) continue;
			else{
				this->DamageApply(*e, damage);
			}
		}
	}
}


// すべての敵にダメージを与える /
void C_ENEMY::Damage4(int damage)
{
	int				i;

	Boss.Damage4(damage);

	for(i=0;i<EnemyNow;i++){
		auto* e = &Enemy[EnemyInd[i]];
		if(e->flag&EF_DAMAGE){
			if(e->flag==EF_BOMB || !(e->flag&EF_DAMAGE)) continue;
			else{
				this->DamageApply(*e, damage);
				//return true;
			}
		}
	}

	//return false;
}


// 敵データを初期化する(x,y は x64 で指定のこと) //
void C_ENEMY::InitEnemyDataX64(ENEMY_DATA *e, int x, int y, uint32_t id)
{
	e->x = x;
	e->y = y;

	const uint32_t addr = (4 + (id << 2)); // ちと、やばいね...
	this->ECL_LongJump(e, addr);

	e->hp       = 0xffffffff;
	e->amp      = 0;
	e->anm_ptn  = 0;
	e->anm_ptnEx= 0;	// 追加 : 2000/11/27 (ダメージ中のアニメ)
	e->anm_sp   = 0;
	e->anm_c    = 0;
	e->count    = 0;
	e->evscore  = 0;
	e->d        = 64;
	e->flag     = EF_DAMAGE|EF_DRAW|EF_HITSB;

	e->IsDamaged = 0;

	e->tama_c   = Cast::down<uint8_t>(RNG.next()); // & 0xff;
	e->g_width  = 0;
	e->g_height = 0;

	e->item     = ITEM_SCORE;

	e->v        = 64;
	e->vd       = 0;
	e->vx       = cosl(e->d,e->v);
	e->vy       = sinl(e->d,e->v);

	e->LLaserRef = 0;

	e->t_cmd.c      = 0;
	e->t_cmd.cmd    = TC_WAY;
	e->t_cmd.d      = 64;
	e->t_cmd.n      = 1;
	e->t_cmd.option = TE_NONE;
	e->t_cmd.type   = T_NORM;
	e->t_cmd.v      = 3;
	e->t_cmd.x      = 0;
	e->t_cmd.y      = 0;

	e->t_cmd.dw     = 16;
	e->t_cmd.ns     = 1;
	e->t_cmd.rep    = 0;
	e->t_cmd.vd     = 0;

	e->l_cmd.l2   = 0;
	e->l_cmd.x    = 0;
	e->l_cmd.y    = 0;
	e->l_cmd.notr = 0xff;

	// 変数用レジスタの初期化 //
	e->GR[0] = e->GR[1] = e->GR[2] = e->GR[3] = 0;
	e->GR[4] = e->GR[5] = e->GR[6] = e->GR[7] = 0;

	// 割り込みベクタの初期化 //
	for(auto& it : e->Vect) {
		it.vect = 0;
	}
}


// 強制的に ECL ブロック間を移動する //
void C_ENEMY::ECL_LongJump(ENEMY_DATA *e, uint32_t EclID) const
{
	e->cmd = U32LEAt(&ECL_Head[EclID]);

	e->call_addr = e->cmd;

	e->t_rep    = 0;			// 弾の発射間隔(０：自動発射しない)
	e->rep_c    = 0;
	e->cmd_c    = 0;
}


// 敵データを初期化する(x,y は非x64(ランダム可能) で指定のこと) //
void C_ENEMY::InitEnemyDataSTD(ENEMY_DATA *e, short x, short y, uint32_t id)
{
/*
	e->x   = I16LEAt(&p[0]);	// PixelToWorld(I16LEAt(&p[0]));
	e->y   = I16LEAt(&p[2]);	// PixelToWorld(I16LEAt(&p[2]));
*/

	// ランダム配置に対応するぞ //
	const int EnemyX = PixelToWorld(
		(x == X_RNDV) ? (X_MIN + (RNG.next() % (X_MAX - X_MIN))) : x
	);
	const int EnemyY = PixelToWorld(
		(y == Y_RNDV) ? (Y_MIN + (RNG.next() % (Y_MAX - Y_MIN))) : y
	);

	InitEnemyDataX64(e, EnemyX, EnemyY, id);
}

void C_ENEMY::AnimeMove(ENEMY_DATA *e) const
{
	const auto *a = &Anime[e->anm_ptn];

	switch(a->mode){
		case(ANM_NORM):
			if(e->anm_sp>0 && (e->count%e->anm_sp==0))
				e->anm_c = (e->anm_c+1)%(a->n);
			else if(e->anm_sp<0 && (e->count%(-e->anm_sp)==0))
				e->anm_c = (e->anm_c+a->n-1)%(a->n);
		break;

		// 逆方向は不可としておきましょうか... //
		case(ANM_STOP):
			if(e->anm_sp>0 && (e->count%e->anm_sp==0)){
				if(e->anm_c < (a->n-1)) e->anm_c++;
			}
		break;

		default:
		break;
	}
}

void C_ENEMY::Delete(ENEMY_DATA& e)
{
	if(e.LLaserRef) {
		LLaser.ForceClose(&e); // レーザーの強制クローズ
	}
	e.flag = EF_DELETE;
}

void C_ENEMY::Explode(ENEMY_DATA& e)
{
	if(e.LLaserRef) {
		LLaser.ForceClose(&e); // レーザーの強制クローズ
	}
	e.hp = 0;
	e.count = 0; // 爆発のアニメセット用
	e.flag = EF_BOMB;
}

void C_ENEMY::ECL_Parse(ENEMY_DATA *e)
{
	bool bRetFlag = false; // 実行クロック０命令の場合はfalseにすること
	int		RegCmp;
	HLaserInfo		HInfo;

	auto& TamaCmd = Tama.TamaCmd;
	const PIXEL_LTRB rcDegX2 =
		{GX_MIN+150*64, GY_MIN+(GY_MID-GY_MIN-40*64)/3,
			GX_MAX-150*64, GY_MID-(GY_MID-GY_MIN-40*64)/3 - 40*64};
	uint16_t	BaseAngle;
	uint16_t	DeltaAngle;

	CheckECLInterrupt(Boss, e);

	while(!bRetFlag) {
		// Note that we need all three flags to express all possible control
		// flow variants and parsing states:
		//
		// • (([stay] == false) && ([bRetFlag] == false)): Advance [e->cmd],
		//   keep running (most common case)
		//
		// • (([stay] == false) && ([bRetFlag] == true)): Advance [e->cmd], but
		//   return after this instruction (rarely used)
		//
		// • (([stay] == true) && ([bRetFlag] == true)): Don't advance
		///  [e->cmd], return after this instruction (used to be a `return` in
		//   pbg's original code)
		//
		// • (([stay] == true) && ([bRetFlag] == false)): Don't advance
		///  [e->cmd], immediately continue at instruction pointer (required
		//   for jumps and calls)
		//
		// • (([stay] == false) && ([error] == true)): Invalid parameter, but
		//   we can keep parsing; keep running based on [bRetFlag]
		//
		// • (([stay] == true) && ([error] == true)): Invalid instruction,
		//   cannot continue parsing
		bool stay = false;
		bool error = false;

		const auto goto_top_of_loop = [&]{
			stay = true;
		};
		const auto stop_for_this_frame = [&]{
			stay = true;
			bRetFlag = true;
		};

		auto *cmd = (ECL_Head.get() + e->cmd);

		switch(*cmd) {
		case(ECL_CEFC):
			break;

		case(ECL_XYRND): {
			const auto rnd_x = ((RNG.next() % (X_MAX - X_MIN - 100)) * 32);
			if(e->x > GX_MID){
				e->x = (PixelToWorld(X_MID) - rnd_x);
			}
			else{
				e->x = (PixelToWorld(X_MID) + rnd_x);
			}

			e->y = (
				PixelToWorld(RNG.next() % (Y_MID - Y_MIN - 160)) +
				PixelToWorld(Y_MIN + 40)
			);
		}
		break;

		case(ECL_XYL):	// 1+2 Bytes Param
			e->x += cosl(e->d, PixelToWorld(I16LEAt(&cmd[1])));
			e->y += sinl(e->d, PixelToWorld(I16LEAt(&cmd[1])));
		break;

		case(ECL_STG4EFC):
			switch(cmd[1]){
			case(STG4ROCK_STDMOVE):
			case(STG4ROCK_ACCMOVE1):
			case(STG4ROCK_3DMOVE):
			case(STG4ROCK_LEAVE):
			case(STG4ROCK_END):
				Effect3D.SendCmdStg4Rock(cmd[1], 0);
				break;
			case(STG4ROCK_ACCMOVE2):
				Effect3D.SendCmdStg4Rock(cmd[1], e->d);
				break;
			}
		break;

		case(ECL_STG3EFC):
			Effect3D.Set(EFFECT3D_TYPE::S3_STAR);
			bRetFlag = true;
		break;

		case(ECL_ITEM):
			e->item = cmd[1];
			bRetFlag = true;
		break;

		case(ECL_HITXY):		// 当たり判定を変更する
			e->g_width  = PixelToWorld(U16LEAt(&cmd[1]));
			e->g_height = PixelToWorld(U16LEAt(&cmd[3]));
		break;

		case(ECL_HLASER):		// ホーミングレーザーセット
			HInfo.c    = e->l_cmd.c;
			HInfo.d    = e->l_cmd.d;
			HInfo.dw   = e->l_cmd.dw;
			HInfo.n    = e->l_cmd.n;
			HInfo.type = e->l_cmd.type;
			HInfo.x    = e->x + e->l_cmd.x;
			HInfo.y    = e->y + e->l_cmd.y;
			HLaser.Set(&HInfo);
			bRetFlag = true;
		break;

		case(ECL_LLSET): { // 太レーザーセット
			auto& LLaserCmd = LLaser.LLaserCmd;
			LLaserCmd.c  = e->l_cmd.c;
			LLaserCmd.d  = e->l_cmd.d;
			LLaserCmd.dx = e->l_cmd.x;
			LLaserCmd.dy = e->l_cmd.y;
			LLaserCmd.e  = e;
			LLaserCmd.type = e->l_cmd.type;
			//LLaserCmd.type = (e->l_cmd.type==0) ? LLS_LONG : LLS_SETDEG;
			LLaserCmd.v = e->l_cmd.v;
			LLaserCmd.w = e->l_cmd.w;

			// 失敗した場合は、参照カウントを増やさない //
			if(LLaser.Set(e->LLaserRef)) {
				e->LLaserRef++;
			}
		}
		break;

		case(ECL_LLOPEN):		// 太レーザーオープン cmd,id
			LLaser.Open(e,cmd[1]);
		break;

		case(ECL_LLCLOSE):		// 太レーザークローズ(消去＆参照カウント減少) cmd,id
			LLaser.Close(e,cmd[1]);
			if(cmd[1]==ECLCST_LLASERALL) e->LLaserRef =  0;
			else                         e->LLaserRef -= 1;		// ちょっとバグ有りなので注意
		break;

		case(ECL_LLCLOSEL):		// 太レーザーライン状態へ cmd,id
			LLaser.Line(e,cmd[1]);
		break;

		case(ECL_LLDEGR):		// 太レーザー角度相対変更 cmd,id,deg
			// 順番が逆だから注意ね
			LLaser.DegR(e, Cast::sign<int8_t>(cmd[2]), cmd[1]);
		break;

		case(ECL_SETUP):		// 敵の初期化
			e->hp    = U32LEAt(&cmd[1 + 0]);
			e->score = U32LEAt(&cmd[1 + 4]);
			if(e->hp == 0) {
				Boss.KillAll();
			}
		break;

		case(ECL_END):			// 敵の強制消滅
			this->Delete(*e); // 後で変更するように
			stop_for_this_frame(); // バグ防止(かも)
		break;

		case(ECL_JMP):			// ◎ＥＣＬ無条件ジャンプ(少々特殊な動作をします)
			e->cmd = U32LEAt(&cmd[1]);
			goto_top_of_loop();
		break;

		case(ECL_LOOP):			// ◎一定区間を繰り返す
			if(e->rep_c == 0) {
				e->rep_c = (U16LEAt(&cmd[1 + 4]) + 1);
			}
			if((--e->rep_c)!=0){
				e->cmd = U32LEAt(&cmd[1]);
				goto_top_of_loop();
			}
		break;

		case(ECL_CALL):			// ＠サブルーチンを呼ぶ
			e->call_addr = e->cmd + ECL_CmdLen[ECL_CALL];
			e->cmd       = U32LEAt(&cmd[1]);
			goto_top_of_loop();
		break;

		case(ECL_RET):			// ＠サブルーチンから復帰する
			e->cmd = e->call_addr;
			goto_top_of_loop();
		break;

		case(ECL_JHPL): { // ◎ＨＰが指定値より大きければジャンプ
			const auto target = U32LEAt(&cmd[1]);
			if(e->hp > U32LEAt(&cmd[1 + 4])) {
				e->cmd = target;
				goto_top_of_loop();
			}
		}
		break;

		case(ECL_JHPS): { // ◎ＨＰが指定値より小さければジャンプ
			const auto target = U32LEAt(&cmd[1]);
			if(e->hp < U32LEAt(&cmd[1 + 4])) {
				e->cmd = target;
				goto_top_of_loop();
			}
		}
		break;

		case(ECL_JDIF):			// 難易度によるジャンプ
			switch(PlayRank.LevelRanked()) {
				case(GAME_EASY):	e->cmd = U32LEAt(&cmd[1 +  0]);	break;
				default:
				case(GAME_NORMAL):	e->cmd = U32LEAt(&cmd[1 +  4]);	break;
				case(GAME_HARD):	e->cmd = U32LEAt(&cmd[1 +  8]);	break;
				case(GAME_LUNATIC):	e->cmd = U32LEAt(&cmd[1 + 12]);	break;
			}
			goto_top_of_loop();
		break;

		case(ECL_JDSB): { // 自機と進行角が一致したらジャンプ
			const uint8_t temp = abs(
				atan8((Viv.x - e->x), (Viv.y - e->y)) - (e->d)
			);
			if(temp<4){
				e->cmd = U32LEAt(&cmd[1]);
				goto_top_of_loop();
			}
		}
		break;

		case(ECL_JFCL):			// フレームカウンタが大きければジャンプ
			if(e->count > U32LEAt(&cmd[1 + 4])) {
				e->cmd = U32LEAt(&cmd[1]);
				goto_top_of_loop();
			}
		break;

		case(ECL_JFCS):			// フレームカウンタが小さければジャンプ
			if(e->count < U32LEAt(&cmd[1 + 4])) {
				e->cmd = U32LEAt(&cmd[1]);
				goto_top_of_loop();
			}
		break;

		case(ECL_STI):			// 割り込みベクタをセットする Addr(4),条件(1),比較値(4)
			switch(cmd[1+4]){
				case(ECLVECT_BITLEFT):
					e->Vect[ECLVECT_BITLEFT].vect  = U32LEAt(&cmd[1]);
					e->Vect[ECLVECT_BITLEFT].value = U32LEAt(&cmd[1 + 4 + 1]);
				break;

				case(ECLVECT_BOSSLEFT):
					e->Vect[ECLVECT_BOSSLEFT].vect  = U32LEAt(&cmd[1]);
					e->Vect[ECLVECT_BOSSLEFT].value = U32LEAt(&cmd[1 + 4 + 1]);
				break;

				case(ECLVECT_HP):
					e->Vect[ECLVECT_HP].vect  = U32LEAt(&cmd[1]);
					e->Vect[ECLVECT_HP].value = U32LEAt(&cmd[1 + 4 + 1]);
				break;

				case(ECLVECT_TIMER):
					e->Vect[ECLVECT_TIMER].vect  = U32LEAt(&cmd[1]);
					e->Vect[ECLVECT_TIMER].value = U32LEAt(&cmd[1 + 4 + 1]);
					e->IntTimer = 0;
				break;

				default:
					error = true;
				break;
			}
		break;

		case(ECL_CLI):			// 割り込みベクタをクリアする
			switch(cmd[1]){
				case(ECLVECT_BITLEFT):
					e->Vect[ECLVECT_BITLEFT].vect  = 0;
				break;

				case(ECLVECT_BOSSLEFT):
					e->Vect[ECLVECT_BOSSLEFT].vect  = 0;
				break;

				case(ECLVECT_HP):
					e->Vect[ECLVECT_HP].vect  = 0;
				break;

				case(ECLVECT_TIMER):
					e->Vect[ECLVECT_TIMER].vect = 0;
				break;
			}
		break;

		case(ECL_NOP):			// ＠何もしない
			if(e->cmd_c == 0) {
				e->cmd_c = (U16LEAt(&cmd[1]) + 1);
			}
			if((--e->cmd_c) != 0) {
				stop_for_this_frame();
			}
		break;

		case(ECL_NOPSC):		// スクロールに流される
			if(e->cmd_c == 0) {
				e->cmd_c = (U16LEAt(&cmd[1]) + 1);
			}
			if((--e->cmd_c)!=0){
				// スクロールに流される処理を記述 //
				stop_for_this_frame();
			}
		break;

		case(ECL_T2ITEM):		// 弾の何％かをアイテム化する
			Tama.ToItem(cmd[1]);
		break;

		case(ECL_ACC):			// 加速移動
			if(e->cmd_c == 0){
				// 初期化 //
				e->cmd_c = (U16LEAt(&cmd[2]) + 1);
				//e->vx    = cosl(e->d,e->v);
				//e->vy    = sinl(e->d,e->v);
			}
			if((--e->cmd_c)!=0){
				e->v += Cast::sign<int8_t>(cmd[1]);
				e->x += cosl(e->d, e->v);
				e->y += sinl(e->d, e->v);
				stop_for_this_frame();
			}
			// 最後は、何もしない訳で... //
		break;

		case(ECL_ACCXYA):		// ＸＹ指定加速移動
			// ちょっと、待ってね //
			bRetFlag = true;
		break;

		case(ECL_DEGX2):		// 制限付き角度ランダム
			if(e->y < rcDegX2.top){
				if(e->x < rcDegX2.left){
					// 左上 //
					BaseAngle  = 32-16;//0;
					DeltaAngle = 32;//64;
				}
				else if(e->x > rcDegX2.right){
					// 右上 //
					BaseAngle  = 96-16;//64;
					DeltaAngle = 32;//64;
				}
				else{
					// 上端 //
					// BaseAngle  = (24 + ((RNG.next() >> 1) % (64 - 16)) - 16); //0;
					BaseAngle  = (32 + (((RNG.next() >> 1) & 1) * 64) - 16);
					DeltaAngle = 32;//128;
				}
			}
			else if(e->y > rcDegX2.bottom){
				if(e->x < rcDegX2.left){
					// 左下 //
					BaseAngle  = -32-16;//192;
					DeltaAngle = 32;//64;
				}
				else if(e->x > rcDegX2.right){
					// 右下 //
					BaseAngle  = 128+32-16;//128;
					DeltaAngle = 32;//64;
				}
				else{
					// 下端 //
					BaseAngle  = 128+64-16;//128;
					DeltaAngle = 32;//128;
				}
			}
			else{
				if(e->x < rcDegX2.left){
					// 左側 //
					BaseAngle  = -16;//192;
					DeltaAngle = 32;//128;
				}
				else if(e->x > rcDegX2.right){
					// 右側 //
					BaseAngle  = 128-16;//64;
					DeltaAngle = 32;//128;
				}
				else{
					// 真ん中 //
					BaseAngle = (((RNG.next() >> 1) & 1) ? (-16) : (128 - 16));
					DeltaAngle = 32;
				}
			}

			// 実際に角度を確定する //
			e->d = (BaseAngle + ((RNG.next() >> 1) % DeltaAngle));
		break;

		case(ECL_MOV):			// ＠直線移動
			if(e->cmd_c==0){
				e->cmd_c = (U16LEAt(&cmd[1]) + 1);
				e->vx    = cosl(e->d,e->v);
				e->vy    = sinl(e->d,e->v);
			}
			if((--e->cmd_c)!=0){
				e->x += e->vx;
				e->y += e->vy;
				stop_for_this_frame();
			}
		break;

		case(ECL_ROL):			// ＠回転移動
			if(e->cmd_c==0){
				e->cmd_c = (U16LEAt(&cmd[1 + 1]) + 1);
				e->vd    = REL_DEGRL(Cast::sign<int8_t>(cmd[1]));
			}
			if((--e->cmd_c)!=0){
				e->x += cosl(e->d,e->v);
				e->y += sinl(e->d,e->v);
				e->d += e->vd;
				stop_for_this_frame();
			}
		break;

		case(ECL_LROL):			// ＠回転＆直線移動
			if(e->cmd_c==0){
				e->cmd_c = (U16LEAt(&cmd[1 + 9]) + 1);
				e->vx    = ABS_VXRL(I32LEAt(&cmd[1]));
				e->vy    = I32LEAt(&cmd[1+4] );
				e->vd    = REL_DEGRL((char)cmd[1+8]);
			}
			if((--e->cmd_c)!=0){
				e->x += (cosl(e->d,e->v)+e->vx);
				e->y += (sinl(e->d,e->v)+e->vy);
				e->d += e->vd;
				stop_for_this_frame();
			}
		break;

		case(ECL_WAVX):			// ＠波Ｘ移動
			if(e->cmd_c==0){
				e->cmd_c = (U16LEAt(&cmd[1 + 6]) + 1);
				e->vx    = ABS_VXRL(I32LEAt(&cmd[1]));
				e->vy    = e->y;
				e->amp   = cmd[1 + 4];
				e->vd    = Cast::sign<int8_t>(cmd[1 + 5]);
				//e->d     = 0;
			}
			if((--e->cmd_c)!=0){
				e->x += e->vx;
				e->y  = e->vy + sinl(e->d,e->amp<<6);
				e->d += e->vd;
				stop_for_this_frame();
			}
		break;

		case(ECL_WAVY):			// ＠波Ｙ移動
			if(e->cmd_c==0){
				e->cmd_c = (U16LEAt(&cmd[1 + 6]) + 1);
				e->vy    = I32LEAt(&cmd[1]);
				e->vx    = e->x;
				e->amp   = cmd[1 + 4];
				e->vd    = Cast::sign<int8_t>(cmd[1 + 5]);
				//e->d     = 0;
			}
			if((--e->cmd_c)!=0){
				e->y += e->vy;
				e->x  = e->vx + sinl(e->d,e->amp<<6);
				e->d += e->vd;
				stop_for_this_frame();
			}
		break;

		case(ECL_MXA):		// Ｘ絶対移動
			if(e->cmd_c==0){
				e->cmd_c = (U16LEAt(&cmd[1 + 2]) + 1);
				e->vx    = ((PixelToWorld(U16LEAt(&cmd[1])) - e->x) / e->cmd_c);
				e->vy    = 0;
			}
			if((--e->cmd_c)!=0){
				e->x += e->vx;
				stop_for_this_frame();
			}
		break;

		case(ECL_MYA):		// Ｙ絶対移動
			if(e->cmd_c==0){
				e->cmd_c = (U16LEAt(&cmd[1 + 2])) + 1;
				e->vy    = ((PixelToWorld(U16LEAt(&cmd[1])) - e->y) / e->cmd_c);
				e->vx    = 0;
			}
			if((--e->cmd_c)!=0){
				e->y += e->vy;
				stop_for_this_frame();
			}
		break;

		case(ECL_MXYA):		// ＸＹ絶対移動
			if(e->cmd_c==0){
				e->cmd_c = (U16LEAt(&cmd[1 + 4]) + 1);
				e->vx    = ((PixelToWorld(U16LEAt(&cmd[1 + 0])) - e->x) / e->cmd_c);
				e->vy    = ((PixelToWorld(U16LEAt(&cmd[1 + 2])) - e->y) / e->cmd_c);
			}
			if((--e->cmd_c)!=0){
				e->x += e->vx;
				e->y += e->vy;
				stop_for_this_frame();
			}
		break;

		case(ECL_MXS):		// Ｘサボテンセット移動
			if(e->cmd_c==0){
				e->cmd_c = (U16LEAt(&cmd[1]) + 1);
				e->vx    = ((Viv.x)-(e->x))/e->cmd_c;
				e->vy    = 0;
			}
			if((--e->cmd_c)!=0){
				e->x += e->vx;
				stop_for_this_frame();
			}
		break;

		case(ECL_MYS):		// Ｙサボテンセット移動
			if(e->cmd_c==0){
				e->cmd_c = (U16LEAt(&cmd[1]) + 1);
				e->vx    = 0;
				e->vy    = ((Viv.y)-(e->y))/e->cmd_c;
			}
			if((--e->cmd_c)!=0){
				e->y += e->vy;
				stop_for_this_frame();
			}
		break;

		case(ECL_MXYS):		// ＸＹサボテンセット移動
			if(e->cmd_c==0){
				e->cmd_c = (U16LEAt(&cmd[1]) + 1);
				e->vx    = ((Viv.x)-(e->x))/e->cmd_c;
				e->vy    = ((Viv.y)-(e->y))/e->cmd_c;
			}
			if((--e->cmd_c)!=0){
				e->x += e->vx;
				e->y += e->vy;
				stop_for_this_frame();
			}
		break;

		case(ECL_GRAX):		// 重力付きＸ反射移動(Y_MIN 含むけど...)
			// 注意：この命令から脱出する方法は割り込み以外に存在しない //
			if(e->cmd_c == 0){
				e->cmd_c = 9999;	// 非ゼロ値であれば、どのような値でも良い
				e->vx    = cosl(e->d, e->v);
				e->vy    = sinl(e->d, e->v);
				e->vd    = Cast::sign<int8_t>(cmd[1]);	// 重力加速度!!
				e->flag |= EF_CLIP;				// クリッピング属性を自動的にセットする
			}
			else{
				e->x += e->vx;
				e->y += e->vy;
				e->vy += e->vd;

				// Ｘ方向のチェック //
				if((e->x) < GX_MIN || (e->x) > GX_MAX){
					e->vx = -(e->vx);	// 速度反転
					e->x += e->vx;
				}
				// Ｙ方向(上)のチェック //
				if((e->y) < GY_MIN){
					e->vy = -(e->vy);	// 速度を反転するのです
					e->y += e->vy;
				}
				// Ｙ方向(下)のチェック -> さよならですな //
				// この部分だけ、縦方向判定を広く取るのだ //
				if((e->y) > GY_MAX+(e->g_height)){
					e->flag = EF_DELETE;	// 消えておしまい
				}
			}
			stop_for_this_frame();
		break;

		case(ECL_DEGA):			// ＠角度絶対指定
			e->d = ABS_DEGRL(cmd[1]);
		break;

		case(ECL_DEGR):			// ＠角度相対指定
			e->d += REL_DEGRL(Cast::sign<int8_t>(cmd[1]));
		break;

		case(ECL_DEGX):			// ＠角度ランダムセット
			e->d = (RNG.next() & 0xff);
		break;

		case(ECL_DEGXU):		// ＠角度ランダムセット(上)
			e->d = (128 + (RNG.next() & 0x7f));
		break;

		case(ECL_DEGXD):		// ＠角度ランダムセット(下)
			e->d = (RNG.next() & 0x7f);
		break;

		case(ECL_DEGEX):
			e->d = EnemyEXDEG;
			EnemyEXDEG += EnemyEXDEG_D;
		break;

		case(ECL_DEGS):			// ＠角度自機セット
			e->d = atan8(Viv.x-e->x,Viv.y-e->y);
		break;

		case(ECL_SPDA):			// ＠速度絶対指定
			e->v = I32LEAt(&cmd[1]);
		break;

		case(ECL_SPDR):			// ＠速度相対指定
			e->v += I32LEAt(&cmd[1]);
		break;

		case(ECL_XYA):			// ＠座標絶対指定
			e->x     = PixelToWorld(I16LEAt(&cmd[1 + 0]));
			e->y     = PixelToWorld(I16LEAt(&cmd[1 + 2]));
		break;

		case(ECL_XYR):			// ＠座標相対指定
			e->x += PixelToWorld(I16LEAt(&cmd[1 + 0]));
			e->y += PixelToWorld(I16LEAt(&cmd[1 + 2]));
		break;

		case(ECL_XYS):
			e->x = Viv.x;
			e->y = Viv.y;
		break;

		case(ECL_TAMA):			// ＠弾発射
			TamaCmd = e->t_cmd;
			TamaCmd.x += e->x;
			TamaCmd.y += e->y;
			Tama.Set();
		break;

		case(ECL_TAMA2):		// ＠弾発射(難易度変化なし)
			TamaCmd = e->t_cmd;
			TamaCmd.x += e->x;
			TamaCmd.y += e->y;
			Tama.SetEX();
		break;

		case(ECL_TAMAL):		// ライン状に弾を発射する
			TamaCmd = e->t_cmd;
			TamaCmd.x += e->x;
			TamaCmd.y += e->y;
			Tama.SetLine();
		break;

		case(ECL_TAMAEX):
			TamaCmd = e->t_cmd;
			TamaCmd.x += e->x;
			TamaCmd.y += e->y;
			Tama.SetExtra01();
		break;

		case(ECL_TAUTO):		// 弾発射モード変更
			e->t_rep = cmd[1];
		break;

		case(ECL_TXYR):			// ＠弾発射位置相対指定
			e->t_cmd.x = PixelToWorld(I16LEAt(&cmd[1 + 0]));
			e->t_cmd.y = PixelToWorld(I16LEAt(&cmd[1 + 2]));
		break;

		case(ECL_TCMD):			// ＠弾コマンド
			e->t_cmd.cmd = cmd[1];
		break;

		case(ECL_TDEGA):		// ＠弾発射角絶対指定
			e->t_cmd.d  = cmd[1];
			e->t_cmd.dw = cmd[1 + 1];
		break;

		case(ECL_TDEGR):		// ＠弾発射角相対指定
			e->t_cmd.d  += Cast::sign<int8_t>(cmd[1]);
			e->t_cmd.dw += Cast::sign<int8_t>(cmd[1 + 1]);
		break;

		case(ECL_TDEGS):		// ＠弾発射角サボテンセット
			// 正確には、TamaCmd の x,y も使うべきだが...
			e->t_cmd.d = atan8(Viv.x-e->x,Viv.y-e->y);
		break;

		case(ECL_TDEGE):		// ＠弾発射角の同期をとる
			e->t_cmd.d = e->d;
		break;

		case(ECL_TNUMA):		// ＠弾発射数絶対指定
			e->t_cmd.n  = cmd[1];
			e->t_cmd.ns = cmd[1 + 1];
		break;

		case(ECL_TNUMR):		// ＠弾発射数相対指定
			e->t_cmd.n  += Cast::sign<int8_t>(cmd[1]);
			e->t_cmd.ns += Cast::sign<int8_t>(cmd[1 + 1]);
		break;

		case(ECL_TSPDA):		// ＠弾速度絶対指定
			e->t_cmd.v = cmd[1];
			e->t_cmd.a = Cast::sign<int8_t>(cmd[1 + 1]);
		break;

		case(ECL_TSPDR): { // ＠弾速度相対指定
			const auto temp = e->t_cmd.v;

			// フラグを外して演算
			e->t_cmd.v = (((temp & 0x3f) + Cast::sign<int8_t>(cmd[1])) & 0x3f);

			e->t_cmd.v |= (temp&0xc0);//(temp&0x3c);						// フラグを書き戻す
			e->t_cmd.a += Cast::sign<int8_t>(cmd[1 + 1]);
		}
		break;

		case(ECL_TOPT):			// ＠弾オプション
			e->t_cmd.option = cmd[1];
		break;

		case(ECL_TTYPE):		// ＠弾タイプ
			e->t_cmd.type = cmd[1];
		break;

		case(ECL_TCOL):			// ＠弾の色もしくは形状
			e->t_cmd.c = cmd[1];
		break;

		case(ECL_TVDEG):		// ＠弾の角速度
			e->t_cmd.vd = Cast::sign<int8_t>(cmd[1]);
		break;

		case(ECL_TREP):			// ＠弾の REP 指定
			e->t_cmd.rep = cmd[1];
		break;

		case(ECL_TCLR):			// 敵弾を全消去(レーザー含む)
			Boss.ClearCmd(); // この処理を何よりも優先させる(ビット消去等を含む)
			Tama.Clear();
			Laser.Clear();
			HLaser.Clear();
			this->Clear();
		break;

		case(ECL_LASER):		// レーザー発射
			Laser.LaserCmd = e->l_cmd;
			Laser.LaserCmd.x += e->x;
			Laser.LaserCmd.y += e->y;
			Laser.Set();
		break;

		case(ECL_LASER2):		// レーザー発射
			Laser.LaserCmd = e->l_cmd;
			Laser.LaserCmd.x += e->x;
			Laser.LaserCmd.y += e->y;
			Laser.SetEX();
		break;

		case(ECL_LCMD):			// レーザーコマンドセット
			e->l_cmd.cmd = cmd[1];
		break;

		case(ECL_LLA):			// レーザー長・絶対指定
			e->l_cmd.l = I32LEAt(&cmd[1]);
		break;

		case(ECL_LLR):			// レーザー長・相対指定
			e->l_cmd.l += I32LEAt(&cmd[1]);
		break;

		case(ECL_LL2):			// レーザー発射位置
			e->l_cmd.l2 = I32LEAt(&cmd[1]);
		break;

		case(ECL_LDEGA):		// レーザー発射角＆幅絶対指定
			e->l_cmd.d  = cmd[1];
			e->l_cmd.dw = cmd[2];
		break;

		case(ECL_LDEGR):		// レーザー発射角＆幅相対指定
			e->l_cmd.d  += Cast::sign<int8_t>(cmd[1]);
			e->l_cmd.dw += Cast::sign<int8_t>(cmd[2]);
		break;

		case(ECL_LDEGS):		// レーザー発射角サボテンセット
			// 正確には、LaserCmd の x,y も使うべきだが...
			e->l_cmd.d = atan8(Viv.x-e->x,Viv.y-e->y);
		break;

		case(ECL_LDEGE):		// レーザー発射角を自分の向きにセット
			e->l_cmd.d = e->d;
		break;

		case(ECL_LNUMA):		// レーザーの本数絶対指定
			e->l_cmd.n = cmd[1];
		break;

		case(ECL_LNUMR):		// レーザーの本数相対指定
			e->l_cmd.n += Cast::sign<int8_t>(cmd[1]);
		break;

		case(ECL_LSPDA):		// レーザーの速度絶対指定
			e->l_cmd.v = I32LEAt(&cmd[1]);
		break;

		case(ECL_LSPDR):		// レーザーの速度相対指定
			e->l_cmd.v = I32LEAt(&cmd[1]);
		break;

		case(ECL_LCOL):			// レーザーの色
			e->l_cmd.c = cmd[1];
		break;

		case(ECL_LTYPE):		// レーザーの種類
			e->l_cmd.type = cmd[1];
		break;

		case(ECL_LWA):			// レーザーの太さ絶対指定
			e->l_cmd.w = I32LEAt(&cmd[1]);
		break;

		case(ECL_LXY):			// レーザーの発射位置指定
			e->l_cmd.x = PixelToWorld(I16LEAt(&cmd[1 + 0]));
			e->l_cmd.y = PixelToWorld(I16LEAt(&cmd[1 + 2]));
		break;

		case(ECL_DRAW_ON):		// ＠描画する
			e->flag |= EF_DRAW;
		break;

		case(ECL_DRAW_OFF):		// ＠描画しない
			e->flag &= (~EF_DRAW);
		break;

		case(ECL_CLIP_ON):		// ＠画面外消去しない
			e->flag |= EF_CLIP;
		break;

		case(ECL_CLIP_OFF):		// ＠画面外消去する
			e->flag &= (~EF_CLIP);
		break;

		case(ECL_DAMAGE_ON):	// ＠ダメージ有り
			e->flag |= EF_DAMAGE;
		break;

		case(ECL_DAMAGE_OFF):	// ＠ダメージ無し
			e->flag &= (~EF_DAMAGE);
		break;

		case(ECL_HITSB_ON):		// ＠自機との当たり判定有り
			e->flag |= EF_HITSB;
		break;

		case(ECL_HITSB_OFF):	// ＠自機との当たり判定無し
			e->flag &= (~EF_HITSB);
		break;

		case(ECL_RLCHG_ON):		// ＠左右反転有り(左側にいればセット)
			if(e->x<GX_MID) e->flag |=   EF_RLCHG;
			else            e->flag &= (~EF_RLCHG);
		break;

		case(ECL_RLCHG_OFF):	// ＠左右反転無し
			e->flag &= (~EF_RLCHG);
		break;

		case(ECL_ANM):			// アニメーションセット
			// anm_ptnEx をセットするのは互換性を保つための配慮 //
			e->anm_ptn  = e->anm_ptnEx = cmd[1];
			e->anm_sp   = Cast::sign<int8_t>(cmd[2]);
			//if(e->anm_sp==0) e->anm_sp=1;
			e->g_height = (Anime[e->anm_ptn].size.h << 5);
			e->g_width  = (Anime[e->anm_ptn].size.w << 5);
			//if(Anime[e->anm_ptn].size.h>32) e->g_height = (e->g_height<<1)/3;
			//if(Anime[e->anm_ptn].size.w>32) e->g_width  = (e->g_width <<1)/3;
			e->anm_c    = 0;
		break;

		case(ECL_ANMEX):
			// 他のパラメータはいっさい変更しない //
			e->anm_ptnEx = cmd[1];
		break;

		case(ECL_PSE):			// 効果音を鳴らす
			Hooks.Snd_SEPlay(cmd[1], e->x, false, &Hooks);
		break;

		case(ECL_INT):			// ボス用割り込み発生
			Boss.INT(e, cmd[1]);
		break;					// cmd を動かさない

		case(ECL_BITATTACK):	// (ボス特権命令)ビットに攻撃パターンをセットする
			Boss.BitAttack(e, U32LEAt(&cmd[1]));
		break;

		case(ECL_BITLASER):		// (ボス特権命令)ビットにレーザー系コマンドをセットする
			Boss.BitLaser(e, cmd[1]);
		break;

		case(ECL_BITCMD):
			Boss.BitCommand(e, cmd[1], I32LEAt(&cmd[2]));
		break;

		case(ECL_EXDEGD):		// 特殊角度増分変更
			EnemyEXDEG_D = cmd[1];
		break;

		case(ECL_ENEMYSET):		// 敵を雑魚指定で発生させる
		case(ECL_ENEMYSETD):	// ＋角度指定(レジスタ)
		{
			short			x,y;

			auto *new_enemy = FindFree();
			if(!new_enemy) {
				break;
			}

			x = ((e->x >> 6) + I16LEAt(&cmd[1])); // PixelToWorld(I16LEAt(&p[0]));
			y = ((e->y >> 6) + I16LEAt(&cmd[3])); // PixelToWorld(I16LEAt(&p[2]));

			// バグに注意注意！！ //
			if(cmd[0] == ECL_ENEMYSETD){
				InitEnemyDataSTD(new_enemy, x, y, cmd[6]);
				new_enemy->d = ID2Value(e, cmd[5]);
			}
			else{
				InitEnemyDataSTD(new_enemy, x, y, cmd[5]);
			}
		}
		break;

		case(ECL_BOSSSET): { // ボスを発生させる
			Boss.Set(((e->x) >> 6), ((e->y) >> 6), cmd[1]);
		} break;

		case(ECL_MOVR): { // レジスタ<->構造体変数の代入
			const auto dwTemp = ID2Value(e, cmd[2]);
			switch(cmd[1]){
				case(ECLCST_GR0):case(ECLCST_GR1):case(ECLCST_GR2):case(ECLCST_GR3):
				case(ECLCST_GR4):case(ECLCST_GR5):case(ECLCST_GR6):case(ECLCST_GR7):
					e->GR[cmd[1]] = dwTemp;
				break;

				case(ECLCST_LCMD_D):	e->l_cmd.d = dwTemp; break;	// レーザーコマンド(角度)
				case(ECLCST_LCMD_DW):	e->l_cmd.dw= dwTemp; break;	// レーザーコマンド(角度差)
				case(ECLCST_LCMD_N):	e->l_cmd.n = dwTemp; break;	// レーザーコマンド(本数)
				case(ECLCST_LCMD_C):	e->l_cmd.c = dwTemp; break;	// レーザーコマンド(色)
				case(ECLCST_LCMD_L):	e->l_cmd.l = dwTemp; break;	// レーザーコマンド(長さ)
				case(ECLCST_LCMD_V):	e->l_cmd.v = dwTemp; break;	// レーザーコマンド(速度)

				case(ECLCST_TCMD_D):	e->t_cmd.d  = dwTemp; break;	// 弾コマンド(角度)
				case(ECLCST_TCMD_DW):	e->t_cmd.dw = dwTemp; break;	// 弾コマンド(角度差)
				case(ECLCST_TCMD_N):	e->t_cmd.n  = dwTemp; break;	// 弾コマンド(個数)
				case(ECLCST_TCMD_NS):	e->t_cmd.ns = dwTemp; break;	// 弾コマンド(連射数)
				case(ECLCST_TCMD_V):	e->t_cmd.v  = dwTemp; break;	// 弾コマンド(速度)
				case(ECLCST_TCMD_C):	e->t_cmd.c  = dwTemp; break;	// 弾コマンド(色)
				case(ECLCST_TCMD_A):	e->t_cmd.a  = dwTemp; break;	// 弾コマンド(加速度)

				case(ECLCST_TCMD_REP):
					//char buf[100];
					//sprintf(buf,"REP=%d [REG:%d]",dwTemp,cmd[2]);
					// DebugOut(buf);
					e->t_cmd.rep = dwTemp;
				break;	// 弾コマンド(繰り返し)

				case(ECLCST_TCMD_VD):	e->t_cmd.vd = dwTemp; break;	// 弾コマンド(角速度)

				case(ECLCST_ENEMY_X):	e->x = dwTemp;	break;	// 敵のＸ座標
				case(ECLCST_ENEMY_Y):	e->y = dwTemp;	break;	// 敵のＸ座標
				case(ECLCST_ENEMY_D):	e->d = dwTemp;	break;	// 敵の角度

				default:
					bRetFlag = true;
					error = true;
				break;
			}
		}
		break;

		case(ECL_MOVC):		// レジスタ<- 定数(即値)の代入
			switch(cmd[1]){
				case(ECLCST_GR0):case(ECLCST_GR1):case(ECLCST_GR2):case(ECLCST_GR3):
				case(ECLCST_GR4):case(ECLCST_GR5):case(ECLCST_GR6):case(ECLCST_GR7):
					e->GR[cmd[1]] = U32LEAt(&cmd[2]);
				break;

				default:	// レジスタ指定がおかしい
					bRetFlag = true;
					error = true;
				break;
			}
		break;

		case(ECL_INC):		// レジスタ＋１
			switch(cmd[1]){
				case(ECLCST_GR0):case(ECLCST_GR1):case(ECLCST_GR2):case(ECLCST_GR3):
				case(ECLCST_GR4):case(ECLCST_GR5):case(ECLCST_GR6):case(ECLCST_GR7):
					e->GR[cmd[1]]++;
				break;

				default:	// レジスタ指定がおかしい
					bRetFlag = true;
					error = true;
				break;
			}
		break;

		case(ECL_DEC):		// レジスタ－１
			switch(cmd[1]){
				case(ECLCST_GR0):case(ECLCST_GR1):case(ECLCST_GR2):case(ECLCST_GR3):
				case(ECLCST_GR4):case(ECLCST_GR5):case(ECLCST_GR6):case(ECLCST_GR7):
					e->GR[cmd[1]]--;
				break;

				default:	// レジスタ指定がおかしい
					bRetFlag = true;
					error = true;
				break;
			}
		break;

		case(ECL_ADD):		// 加算命令(第２引数はレジスタでなくてもよい)
			switch(cmd[1]){
				case(ECLCST_GR0):case(ECLCST_GR1):case(ECLCST_GR2):case(ECLCST_GR3):
				case(ECLCST_GR4):case(ECLCST_GR5):case(ECLCST_GR6):case(ECLCST_GR7):
					e->GR[cmd[1]] += ID2Value(e, cmd[2]);
				break;

				default:	// レジスタ指定がおかしいに
					bRetFlag = true;
					error = true;
				break;
			}
		break;

		case(ECL_SUB):		// 減算命令(第２引数はレジスタでなくてもよい)
			switch(cmd[1]){
				case(ECLCST_GR0):case(ECLCST_GR1):case(ECLCST_GR2):case(ECLCST_GR3):
				case(ECLCST_GR4):case(ECLCST_GR5):case(ECLCST_GR6):case(ECLCST_GR7):
					e->GR[cmd[1]] -= ID2Value(e, cmd[2]);
				break;

				default:	// レジスタ指定がおかしいに
					bRetFlag = true;
					error = true;
				break;
			}
		break;

		case(ECL_SINL):		// sinl(Gr0,Gr1)
			if(cmd[1]<ECLREG_MAX && cmd[2]<ECLREG_MAX) {
				e->GR[cmd[1]] = sinl(
					Cast::down<uint8_t>(e->GR[cmd[2]]), e->GR[cmd[1]]
				);
			} // else {
				// 本当はswitch()で判別したいが...
			// }
		break;

		case(ECL_COSL):		// cosl(Gr0,Gr1)
			if(cmd[1]<ECLREG_MAX && cmd[2]<ECLREG_MAX) {
				e->GR[cmd[1]] = cosl(
					Cast::down<uint8_t>(e->GR[cmd[2]]), e->GR[cmd[1]]
				);
			} // else {
				// 本当はswitch()で判別したいが...
			// }
		break;

		case(ECL_MOD):		// Gr0 = Gr0 % Const
			switch(cmd[1]){
				case(ECLCST_GR0):case(ECLCST_GR1):case(ECLCST_GR2):case(ECLCST_GR3):
				case(ECLCST_GR4):case(ECLCST_GR5):case(ECLCST_GR6):case(ECLCST_GR7):
					if(U32LEAt(&cmd[2]) != 0) {
						e->GR[cmd[1]] %= U32LEAt(&cmd[2]);
					}
					//else
						// ゼロ除算エラー
					break;

				default:	// レジスタ指定がおかしい
					bRetFlag = true;
					error = true;
				break;
			}
		break;

		case(ECL_RND): // Gr0 = RNG.next()
			switch(cmd[1]){
				case(ECLCST_GR0):case(ECLCST_GR1):case(ECLCST_GR2):case(ECLCST_GR3):
				case(ECLCST_GR4):case(ECLCST_GR5):case(ECLCST_GR6):case(ECLCST_GR7):
					e->GR[cmd[1]] = (
						Cast::up<uint32_t>(RNG.next()) * RNG.next()
					);
				break;

				default:	// レジスタ指定がおかしい
					bRetFlag = true;
					error = true;
				break;
			}
		break;

		case(ECL_CMPR):		// レジスタ～レジスタの比較(Reg0,Reg1)
			if((cmd[1] >= ECLREG_MAX) || (cmd[2] >= ECLREG_MAX)) {
				error = true;
				break;
			}
			RegCmp = (ID2Value(e, cmd[1]) - ID2Value(e, cmd[2]));
		break;

		case(ECL_CMPC):		// レジスタ～定数の比較(Reg,Const)
			if(cmd[1] >= ECLREG_MAX) {
				error = true;
				break;
			}
			RegCmp = (ID2Value(e, cmd[1]) - I32LEAt(&cmd[1 + 1]));
		break;

		case(ECL_JL):		// 比較結果 > 0 ならばジャンプ
			if(RegCmp>0){
				e->cmd = U32LEAt(&cmd[1]);
				goto_top_of_loop();
			}
		break;

		case(ECL_JS):		// 比較結果 < 0 ならばジャンプ
			if(RegCmp<0){
				e->cmd = U32LEAt(&cmd[1]);
				goto_top_of_loop();
			}
		break;

		case(ECL_JEQ):		// 比較結果 == 0 ならばジャンプ
			if(RegCmp == 0){
				e->cmd = U32LEAt(&cmd[1]);
				goto_top_of_loop();
			}
		break;

		default:				// 未定義の命令が発生！！
			error = true;
			stop_for_this_frame();
		break;
		}

		if(Hooks.ecl_hook_flag[cmd[0]] || error) {
			Hooks.ECL_Op(cmd, e, error, &Hooks);
		}

		// 次の命令への準備 //
		if(!stay) {
			e->cmd += ECL_CmdLen[*cmd];
		}
	}
}

// 割り込みジャンプを調べる //
extern void CheckECLInterrupt(const C_BOSS& Boss, ENEMY_DATA *e)
{
	int		i;

	for(i=0;i<ECLVECT_MAX;i++){
		if(e->Vect[i].vect==0) continue;	// 割り込みがかかっていない
		switch(i){
			case(ECLVECT_BITLEFT):	// ビット残り割り込み
				if(Boss.GetBitLeft() <= e->Vect[ECLVECT_BITLEFT].value) {
					e->cmd = e->Vect[ECLVECT_BITLEFT].vect;
					e->cmd_c = 0;		// コマンド繰り返しカウンタ
					e->rep_c = 0;		// LOOP(旧REP)命令カウンタ
					e->t_rep = 0;		// 自動弾発射タイミング(0:自動発射せず)
					return;
				}
			break;

			case(ECLVECT_BOSSLEFT):	// ボス残り割り込み
				if(Boss.NumAlive() <= e->Vect[ECLVECT_BOSSLEFT].value) {
					e->cmd = e->Vect[ECLVECT_BOSSLEFT].vect;
					e->cmd_c = 0;		// コマンド繰り返しカウンタ
					e->rep_c = 0;		// LOOP(旧REP)命令カウンタ
					e->t_rep = 0;		// 自動弾発射タイミング(0:自動発射せず)
					return;
				}
			break;

			case(ECLVECT_HP):	// HPL 割り込み
				//char buf[100];
				//sprintf(buf,"hp = %d",e->hp);
				// DebugOut(buf);
				if(e->hp <= e->Vect[ECLVECT_HP].value){
					e->cmd = e->Vect[ECLVECT_HP].vect;
					e->cmd_c = 0;		// コマンド繰り返しカウンタ
					e->rep_c = 0;		// LOOP(旧REP)命令カウンタ
					e->t_rep = 0;		// 自動弾発射タイミング(0:自動発射せず)
					// DebugOut(u8"受理");
					return;
				}
			break;

			case(ECLVECT_TIMER):	// タイマー割り込み
				if(e->IntTimer > e->Vect[ECLVECT_TIMER].value){
					e->cmd = e->Vect[ECLVECT_TIMER].vect;
					e->cmd_c = 0;		// コマンド繰り返しカウンタ
					e->rep_c = 0;		// LOOP(旧REP)命令カウンタ
					e->t_rep = 0;		// 自動弾発射タイミング(0:自動発射せず)
					e->IntTimer = 0;	// この割り込み特有の初期化
					return;
				}
				else{
					e->IntTimer++;
				}
			break;

			default:
			break;
		}
	}
}

// Ensure that everything we return fits losslessly into the return type. If
// this ever fails due to new script commands that return larger values,
// ID2Value() needs a larger return type.
#pragma warning(error: 4244)

// ECLCST_?? からその値に変換する //
static uint32_t ID2Value(const ENEMY_DATA *e, uint8_t id)
{
	switch(id){
		// レジスタ指定の場合 //
		case(ECLCST_GR0):case(ECLCST_GR1):case(ECLCST_GR2):case(ECLCST_GR3):
		case(ECLCST_GR4):case(ECLCST_GR5):case(ECLCST_GR6):case(ECLCST_GR7):
		return e->GR[id];

		case(ECLCST_LCMD_D):	return e->l_cmd.d;	// レーザーコマンド(角度)
		case(ECLCST_LCMD_DW):	return e->l_cmd.dw;	// レーザーコマンド(角度差)
		case(ECLCST_LCMD_N):	return e->l_cmd.n;	// レーザーコマンド(本数)
		case(ECLCST_LCMD_C):	return e->l_cmd.c;	// レーザーコマンド(色)
		case(ECLCST_LCMD_L):	return e->l_cmd.l;	// レーザーコマンド(長さ)
		case(ECLCST_LCMD_V):	return e->l_cmd.v;	// レーザーコマンド(速度)

		case(ECLCST_TCMD_D):	return e->t_cmd.d;	// 弾コマンド(角度)
		case(ECLCST_TCMD_DW):	return e->t_cmd.dw;	// 弾コマンド(角度差)
		case(ECLCST_TCMD_N):	return e->t_cmd.n;	// 弾コマンド(個数)
		case(ECLCST_TCMD_NS):	return e->t_cmd.ns;	// 弾コマンド(連射数)
		case(ECLCST_TCMD_V):	return e->t_cmd.v;	// 弾コマンド(速度)
		case(ECLCST_TCMD_C):	return e->t_cmd.c;	// 弾コマンド(色)
		case(ECLCST_TCMD_A):	return e->t_cmd.a;	// 弾コマンド(加速度)
		case(ECLCST_TCMD_REP):	return e->t_cmd.rep;	// 弾コマンド(繰り返し)
		case(ECLCST_TCMD_VD):	return e->t_cmd.vd;	// 弾コマンド(角速度)

		case(ECLCST_ENEMY_X):	return e->x;	// 敵のＸ座標
		case(ECLCST_ENEMY_Y):	return e->y;	// 敵のＹ座標
		case(ECLCST_ENEMY_D):	return e->d;	// 敵の角度

		default:
		return 0;
	}
}
