	...
	while(bFlag) {
		...
		case(SCL_EFC):
			...
			case(SEFC_STG6CUBE): // ６面キューブ
				Effect3D.Set(EFFECT3D_S6_CUBE);
				break;
			case(SEFC_STG6RNDECL): // ６面ＥＣＬ羅列
				Effect3D.Set(EFFECT3D_S6_FAKE_ECL);
				break;
			case(SEFC_STG4ROCK): // ４面岩
				Effect3D.Set(EFFECT3D_S4_ROCK);
				break;
			case(SEFC_STG4LEAVE): // ４面岩画面外へ
				if(Effect3D.GetActive() != EFFECT3D_S4_ROCK) {
					break;
				}
				Effect3D.SendCmdStg4Rock(STG4ROCK_LEAVE, 0);
				break;
			case(SEFC_STG6RASTER): // ６面ラスター
				Effect3D.Set(EFFECT3D_S6_RASTER);
				break;
			}
			SCL_Now += 2;
			break;
