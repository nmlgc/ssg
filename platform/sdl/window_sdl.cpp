...
SDL_Window *WndBackend_SDLCreate(const GRAPHICS_PARAMS& params)
{
	...
	uint32_t flags = 0;
		...
		if(name.starts_with(u8"opengl")) {
			...
			// SDL_GL_ResetAttributes() also resets the essential profile mask
			// and version selection attributes, but chooses the target OpenGL
			// version via a hardcoded #ifdef priority list that prefers
			// regular OpenGL over ES 2 over ES 1. If the user requested any of
			// the ES versions, SDL_CreateRenderer() would still recreate the
			// window because the `SDL_GL_CONTEXT_PROFILE_MASK` got set to
			// regular/non-ES OpenGL.
			// So, we're forced to specify the correct flags ourselves after
			// all.
