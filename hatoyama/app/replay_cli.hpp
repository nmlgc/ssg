...
struct ARGS_GIVEN {
	...
	std::span<char *> positionals; // Actually `char8_t *`, but SDL...
	bool quiet = false;
};
...
#pragma warning(push)
#pragma warning(disable: 26826) // f.55
void SDL_logf(SDL_LogPriority priority, const char *const fmt, ...)
{
	va_list va;
	va_start(va, fmt);
...
void SDL_printf(const char *const fmt, auto... args)
{
	SDL_logf(SDL_LOG_PRIORITY_INFO, fmt, args...);
}

REPLAY_CLI_RET SDL_fatalf(const char *const fmt, auto... args)
{
	SDL_logf(SDL_LOG_PRIORITY_CRITICAL, fmt, args...);
	return REPLAY_CLI_RET::FATAL;
}

REPLAY_CLI_RET SDL_errorf(const char *const fmt, auto... args)
{
	SDL_logf(SDL_LOG_PRIORITY_ERROR, fmt, args...);
	return REPLAY_CLI_RET::ERROR;
}

REPLAY_CLI_RET SDL_warnf(const char *const fmt, auto... args)
{
...
)
{
	...
	SDL_printf("Options:\n");
	const auto max_len = (std::ranges::max)(
		std::views::transform(OPTS, [](const auto& opt) {
