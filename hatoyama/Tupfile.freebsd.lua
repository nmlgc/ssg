...
-- We need this library in a non-C++ build step later to avoid a compilation error
-- on FreeBSD with the debug build.
local PRINTF_LINK = BuildPrintf(CONFIG:branch({ cflags = "-fPIC" }))
...
function BuildHatoyamaLogic(constants_cflags)
	local link = { cflags = { "-DFREEBSD" } }
	link.cflags += constants_cflags
	TableExtend(link, HATOYAMA_LINK)

	-- Since Pango/Cairo adds -pthread to a later configuration, the C++
	-- standard library must also be compiled with this flag. FreeBSD also
	-- needs -pthread explicit at link time, unlike Linux.
	--
	...
	local modules_cfg = CONFIG:branch({
		cflags = "-pthread",
		lflags = { "-pthread", "-static-libstdc++", "-static-libgcc" },
	})
