...
)
{
	...
	if(!stage) {
		...
		// which will include packfiles, score files, and the original
		// configuration. All these files will produce a lot of noise if we
		// print an error here...
		if(args.quiet) {
	...
	ssg->RoundInitFromReplayOld(info, stage);
